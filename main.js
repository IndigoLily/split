...
const dtypeCtl   = gui.add(opts, 'deviance type', ['Boolean', 'Smooth']).name('Deviance Type');
const bchanceCtl = gui.add(opts, 'bool chance').name('Boolean Chance').min(0).max(100).step(0.1);
...
gui.add(opts, 'split type', ['\u2E85', 'Y']).name('Split Type');
...
function start() {
    ...
    const deviance = opts['deviance'];
    const stype = opts['split type'];
    const dtype = opts['deviance type'];
    const bchance = opts['bool chance'];
